...
Image KuwaharaFilter(const Image &org, uint32_t window_size = 5) {
    ...
    const uint32_t NumBlock = (std::max(W, H) <= 512 ? 4 : 8);

    struct Block { 
        uint32_t lx, ly, rx, ry; 
    };
    std::vector<Block> blocks(NumBlock * NumBlock);
    for (uint32_t size_h = H / NumBlock, y = 0; y < NumBlock; ++y) {
        for (uint32_t size_w = W / NumBlock, x = 0; x < NumBlock; ++x) {
            Block &block = blocks[y * NumBlock + x];
            block.lx = x * size_w;
            block.ly = y * size_h;
            block.rx = (x + 1 == NumBlock ? W - 1 : (x + 1) * size_w - 1);
            block.ry = (y + 1 == NumBlock ? H - 1 : (y + 1) * size_h - 1);
        }
    }
