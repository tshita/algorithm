...
Ford-Fulkerson method は Zwick の例などのように停止しない場合があるが、Edmonds-Karp algorithm は必ず停止することが保証されている。  
各ステップにおける増加道の弧の数が単調非減少で、 $i$ 回目と $i + |A|$ 回目の反復で得られる増加道の弧の数が真に増加しており、増加道の弧の数が $|V| - 1$ 以下と上に有界なので必ず停止する。詳しい証明は [1] を参照。
