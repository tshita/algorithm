...
Gabow の容量スケーリング法では、 $U = \max_{a \in A} c(a)$ , $K = \lfloor 1 + \log_2 {U} \rfloor$ , $s$-$t$ 流 $f_0 = 0$ として、反復  $k = 1, 2, \ldots, K$ に対して順に以下を実行する。

1. 各 $a \in A$ に対して $u_k(a) = \lfloor u(a) / 2^{K - k} \rfloor$ とする
2. $2 f_{k - 1}$ を初期 $s$ - $t$ 流として、増加道法でネットワーク $(G, u_k)$ の最大 $s$ - $t$ 流 $f_k$ を見つける
